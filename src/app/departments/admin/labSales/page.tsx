...
export default function LabTestSales() {
  const [labTestSales, setLabTestSales] = useState<LabTestSale[]>([]);
  const [filteredSales, setFilteredSales] = useState<LabTestSale[]>([]);
  ...
  // Fetch all lab test sales on component mount
  useEffect(() => {
    const fetchLabTestSales = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/lab-test-sales/list/`
        );
        setLabTestSales(response.data);
        setFilteredSales(response.data);
      } catch (error) {
        console.error("Error fetching lab test sales:", error);
      }
    };
    fetchLabTestSales();
  }, []);

  // Filter lab test sales based on search term and date range
  useEffect(() => {
    let filtered = labTestSales;
  ...
  }, [searchTerm, startDate, endDate, labTestSales]);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Lab Test Sales</h1>
        ...
        <TableBody>
          {currentItems.map((sale) => (
            <TableRow key={sale.id}>
              ...
              <TableCell>{sale.operation_count}</TableCell>
